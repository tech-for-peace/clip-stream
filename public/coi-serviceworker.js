...
if (typeof window === "undefined") {
  ...
  (() => {
    ...
    if (window.crossOriginIsolated) {
      !coi.quiet && console.log("Already cross-origin isolated.");
      return;
    }

    if (!coi.shouldRegister()) {
      !coi.quiet && console.log("Not registering coi service worker.");
      return;
    }
    ...
    // In some environments (e.g. Firefox private mode) registering a service worker may fail.
    n.serviceWorker.register(window.document.currentScript.src).then(
      (registration) => {
        ...
        registration.addEventListener("updatefound", () => {
          !coi.quiet &&
            console.log("Reloading page to activate COI service worker.");
